...
class HttpRequestExecutorTest {
    ...
    fun validRequestIsNormalizedAndSentToTransport() {
        var received: ActionConfig? = null
        val executor = HttpRequestExecutor { config ->
            received = config
            HttpResult(204, "No Content")
        }

        val result = executor.execute(ActionConfig("post", " http://192.168.1.5/hook "))

        assertEquals(ActionConfig("POST", "http://192.168.1.5/hook"), received)
    ...
    fun invalidUrlDoesNotReachTransport() {
        ...
        val result = executor.execute(ActionConfig("GET", "ftp://server/file"))
    ...
    fun transportExceptionBecomesErrorResult() {
        ...
        val result = executor.execute(ActionConfig("GET", "http://localhost/test"))
